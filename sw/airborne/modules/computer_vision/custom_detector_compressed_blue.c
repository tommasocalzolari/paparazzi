...
#define CCRD_BLUE_MIDDLE_ROWS_THRESHOLD_NUM 20
#endif
...
#define CCRD_MAX_BLUE_OBJECTS 32
#endif
#ifndef CCRD_MAX_DIM
  ...
  for (sy = 0U; sy < sh; sy++) {
    ...
    for (sx_pair = 0U; sx_pair + 1U < sw; sx_pair += 2U) {
      ...
      for (k = 0U; k < 2U; k++) {
        ...
        if (yy >= CCRD_BLUE_Y_MIN && yy <= CCRD_BLUE_Y_MAX &&
...
static void compute_risk_map(const struct image_t *img, struct risk_result_t *result)
{
  ...
  for (i = 0U; i < 3U; i++) {
    ...
    result->metrics[i].blocked = result->metrics[i].orange_blocked || result->metrics[i].green_blocked || result->metrics[i].blue_blocked ;
                                                       ...
                                                       uint8_t camera_id __attribute__((unused)))
{
  ...
  nblue = detect_blue_objects(proc, blue_objects);
  t7 = now_us();
